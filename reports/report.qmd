...
SELECT
  ...
  `Income Group`,
  `Country Count`,
  -- Sum of counts for the current region partition
  SUM(`Country Count`) OVER (PARTITION BY `Region`) AS `Sum of Countries in Region`,
  -- Sum of counts for the current income group partition
  SUM(`Country Count`) OVER (PARTITION BY `Income Group`) AS `Sum of Countries by Income`,
  -- Sum of all counts (grand total)
  SUM(`Country Count`) OVER () AS `Total Sum Countries`,
  -- Percentage of the grand total for this specific pair
  (`Country Count` * 100.0 / SUM(`Country Count`) OVER ()) AS `Percent of Total`
FROM
  RegionIncomeCounts
ORDER BY
  `Region`, `Income Group`;
...
with IncomeCategoryStats as (
    select
        `Income Group`,
        count(*) as "Country Count",
        sum(count(*)) over () as "Total Countries"
    from
        wdi_country
    where
        `Income Group` is not null and `Income Group` != ''
...
select
    ...
    "Country Count",
    "Total Countries",
    ("Country Count" * 100.0 / "Total Countries") as "Percentage"
from
    IncomeCategoryStats
order by
    `Income Group`;
